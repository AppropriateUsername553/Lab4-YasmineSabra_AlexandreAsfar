...
class InstructorForm(ttk.Frame):
    ...
    def add_instructor(self):
        try:
            if any(i.instructor_id == self.instructor_id_var.get().strip() for i in self.dm.instructors_data):
                raise ValueError(f"Instructor ID {self.instructor_id_var.get()} already exists.")
            ...
            self.dm.instructors_data.append(instructor)
            #refreshing tables
            if self.instructor_table:
                self.instructor_table.load_data()
            #updating json files
            self.dm.save_data_to_file("starschool_data.json")
            ...
            messagebox.showinfo("Success", f"Instructor {instructor.name} is added successfully.")
