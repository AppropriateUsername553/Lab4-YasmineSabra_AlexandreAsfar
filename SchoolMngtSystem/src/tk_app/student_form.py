...
class StudentForm(ttk.Frame): #hold the student form user interface
    ...
    def add_student(self):
        try:
            if any(s.student_id == self.student_id_var.get().strip() for s in self.dm.students_data):
                raise ValueError(f"Student ID {self.student_id_var.get()} already exists.")
            ...
            self.dm.students_data.append(student)
            ...
            messagebox.showinfo("Success", f"Student {student.name} is added successfully.")
