...
class CourseAssignmentForm(ttk.Frame):
    ...
    def assign_course(self):
        try:         
            ...
            #find teh instrcutor by ID
            input_instructor_id = self.instructor_id_var.get()
            for i in self.dm.instructors_data:
                if i.instructor_id == input_instructor_id:
                        matched_instructor = i
                        break
            if matched_instructor is None:
            ...
            messagebox.showinfo("Success", f"Instructor {matched_instructor.name} assigned to {matched_course.course_name}")
